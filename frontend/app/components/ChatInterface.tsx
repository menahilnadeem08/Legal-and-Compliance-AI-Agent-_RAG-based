      ...
      <div className="flex-1 flex gap-4 px-4 pb-4 overflow-hidden min-h-0">
        ...
        {!sidebarOpen && (
          <div className="flex-1 flex items-center justify-center overflow-hidden">
            <div className="text-center max-w-md px-4">
              ...
              <h2 className="text-3xl font-bold text-white mb-3">Welcome to Chat Assistant</h2>
              ...
              <div className="space-y-4">
                <div className="p-4 rounded-lg bg-cyan-500/10 border border-cyan-500/30 text-left">
                  <p className="text-lg font-semibold text-cyan-300 mb-1">📚 Use Documents</p>
                  <p className="text-sm text-gray-400">Reference your uploaded files</p>
                </div>
                <div className="p-4 rounded-lg bg-cyan-500/10 border border-cyan-500/30 text-left">
                  <p className="text-lg font-semibold text-cyan-300 mb-1">🎯 Get Citations</p>
        ...
        {sidebarOpen && (
          <div className="w-96 flex flex-col animate-slide-in-left min-h-0">
              ...
              {messages.length === 0 ? (
                <div className="flex items-center justify-center h-full text-center">
                  <div>
                    ...
                    <p className="text-xs text-gray-500 mt-2">Ask questions about your documents</p>
                  </div>
                </div>
              ) : (
                <>
                    ...
                    >
                      <div
                        className={`message-bubble ${
                          msg.role === 'user' ? 'message-user max-w-xs' : 'message-assistant max-w-sm'
                        }`}
                      >
                        {msg.role === 'assistant' ? (
                          <div className="prose prose-sm prose-invert max-w-none">
                            <ReactMarkdown
                              components={{
                                h1: ({node, ...props}) => <h1 className="text-lg font-bold mt-3 mb-2 text-cyan-300" {...props} />,
                                h2: ({node, ...props}) => <h2 className="text-base font-bold mt-2 mb-1 text-cyan-300" {...props} />,
                                ul: ({node, ...props}) => <ul className="list-disc ml-4 my-1 space-y-1" {...props} />,
                                li: ({node, ...props}) => <li className="text-foreground-dim text-sm" {...props} />,
                                p: ({node, ...props}) => <p className="my-1 text-foreground-dim text-sm" {...props} />,
                              }}
                            >
                              {msg.content}
                            </ReactMarkdown>
                    ...
                    <div className="flex justify-start animate-fade-in">
                      <div className="message-bubble message-assistant">
                        <div className="flex items-center gap-2">
                          ...
                          <p className="text-foreground-dim text-sm">Analyzing your query...</p>
                        </div>
                      </div>
                    </div>
                  )}
            ...
            <form onSubmit={handleSubmit} className="glass-border flex gap-2 p-3 flex-shrink-0">
              <input
                type="text"
                value={input}
